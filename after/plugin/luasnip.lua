...
-- load this first since it overwrites the snippets table.
if true then
  -- take snippets from "friendly-snippets"
  local has_vsc_snips, vsc_snips = pcall(require, "luasnip/loaders/from_vscode")
  if has_vsc_snips then
...
-- ls.add_snippets("all", { -- NOTE: enable this if you want to see/test this snippets!
ls.add_snippets(nil, {
  ...
  -- third arg,
  snippet("never_show", t "this will never show, if condition is false", {
...
for _, ft_path in ipairs(vim.api.nvim_get_runtime_file("lua/user/snip/ft/*.lua", true)) do
  loadfile(ft_path)()
end
...
-- <c-h> is selecting within a list of options.
